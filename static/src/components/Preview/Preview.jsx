		...
		if(type == 'text'){
			if(data.resource_type == 3){
				return (
					<Row gutter={8}>
					<Col span={16}>
						<a href={`${config.qiniu}/${data.source_url}`} onClick={onCollect} ><Button className={styles.button}>预览</Button></a>
					</Col>
					<Col span={8}>
						<a href={`${config.qiniu}/${data.source_url}`} download={`${config.qiniu}/${data.source_url}`} onClick={onCollect}><Button className={styles.button}>下载</Button></a>
					</Col>
					</Row>
				)
			}
