			...
			validateFields([`title-${formType}-${isSelectContextId}`,`detail-${formType}-${isSelectContextId}`,`category-${formType}-${isSelectContextId}`],(errors, values) =>{
				...
				if(isSelectMenuItem == '1'){
					validateFields([`file-${formType}-${isSelectContextId}`],(errors, values) =>{
						...
						body = {course_name: getFieldValue(`title-${formType}-${isSelectContextId}`), description: getFieldValue(`detail-${formType}-${isSelectContextId}`), category: getFieldValue(`category-${formType}-${isSelectContextId}`), images: getFieldValue(`file-${formType}-${isSelectContextId}`)}
						handleSubmit(body)
					});
				}
